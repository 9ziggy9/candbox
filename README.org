...
You have either gcc or clang pre-installed, so the only
thing you will need is our build tool: /make/.\\
- macOS
  #+BEGIN_SRC console
  brew install make
  #+END_SRC
...
- BSD
  ...
  is not the same thing as the communist GNU `make`
  that most people are familiar with. You will need to install
  [[https://www.freshports.org/devel/gmake/][gmake]] to be consistent with these
  notes.\\
- Windows
